...
#define max_counter 200 //for old queues
...
//edits
void priority_cause(TCB* tcb){

	if(tcb->curr_cause==SCHED_MUTEX && tcb->last_cause==SCHED_MUTEX)
	{
		if(tcb->priority>0 && tcb->priority<max_queues-1)
		{
				tcb->priority++;
		}
	}

	if(tcb->curr_cause==SCHED_IO && tcb->priority>0)
	{
			tcb->priority=0;
	}
	else if(tcb->curr_cause==SCHED_QUANTUM && tcb->priority<max_queues-1)
	{
		tcb->priority++;
	}
}
...
void  wait_func()
{
	...
	for(int i =1; i <max_queues; i++)
	{
		if(!is_rlist_empty(&SCHED[i]))
		{
			...
			{
				helper=rlist_pop_front(&SCHED[i]); //take head
				if(helper==NULL)
				{
					return;
				}
				...
				{
					helper->tcb->counter++;
					if(helper->tcb->counter >=max_counter)
					{
						helper->tcb->priority--;
						helper->tcb->counter=0;
					}
				}
				rlist_push_back(&SCHED[helper->tcb->priority], helper); //put at tail of list in a -1 lower priority
			}
		}
	}
	Mutex_Unlock(&sched_spinlock);
